...
class TicTacToeGUI:
    ...
    def get_best_move(self, max_time=2):  # Set a maximum time for the AI move
        best_move = None
        best_eval = -math.inf
                ...
                for row in range(4):
                    for column in range(4):
                        if self.board.get_value(layer, row, column) == '':
                            ...
                            eval = self.minimax(0, -math.inf, math.inf, True)
    ...
    def minimax(self, depth, alpha, beta, maximizing_player):
        depth=2
        if depth == 0 or self.board.is_win('x') or self.board.is_win('O') or self.board.is_full():
            return 0
                ...
                for row in range(4):
                    for col in range(4):
                        if self.board.get_value(layer, row, col) == '':
                            ...
                            self.board.undo_move(layer, row, col)  # Undo the move
